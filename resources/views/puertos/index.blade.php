...
<table class="table table-dark table-striped table-compact">
    ...
    @foreach($puertos as $puerto)
        <tr>
            <td>{{ $puerto['nombre'] ?? '' }}</td>
            <td class="text-right">
                <a href="{{ route('puertos.edit', $puerto['id']) }}" class="btn btn-xs btn-secondary">Editar</a>
                <form action="{{ route('puertos.destroy', $puerto['id']) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar?');">
                    @csrf
                    @method('DELETE')
